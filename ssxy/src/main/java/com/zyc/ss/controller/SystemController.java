...
public class SystemController {
    ...
    * */
    ...
    public Result<String> headerImgUpload
    (
            @ApiParam("头像文件")
        ...
        if (StringUtils.hasLength(originalFilename)) {
            ...
            String portraitPath = "D:\\workspace\\IdeaProjects\\ssxy\\target\\classes\\public\\upload\\".concat(newFileName);
    ...
    @GetMapping("/getInfo")
    public Result<Map<String, Object>> getInfoByToken(
            @ApiParam("token口令")
        ...
        if (userType == null || !StringUtils.hasLength(String.valueOf(userType))) {
            return Result.fail(map).message("token 有误");
        }
        ...
        switch (loginForm.getUserType()) {
            case 1:
                try {
                    ...
                    if (null != admin) {
                        // 用户的类型和用户id转换成一个密文,以token的形式向客户端反馈
